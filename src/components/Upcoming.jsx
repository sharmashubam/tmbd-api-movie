...
const Upcoming = () => {
    ...
    const requestPopular= `https://api.themoviedb.org/3/movie/upcoming?api_key=${key}&language=en-US&page=${page}`;
        ...
        {movies?.map(function (movie, id) {
            return (
                ...
                to='/detail' key={movie.id} className='text-white text-center items-center mt-8'>
                    <LazyLoadImage
                        className='w-[240px] rounded-sm shadow-2xl h-[370px] mx-auto object-cover cursor-pointer transition duration-500 ease-in-out hover:brightness-75 border-2 hover:border-teal-600 border-teal-500'
